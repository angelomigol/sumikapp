...
class AuthCallbackService {
  ...
  /**
   * @name verifyTokenHash
   * @description Verifies Supabase email/magic link token and redirects accordingly.
   * Handles both web and deep link redirects (e.g., sumikapp://auth-callback).
   */
  async verifyTokenHash(
    request: Request,
    params: {
    ...
    // fix host if running outside of vercel / local
    if (url.host.includes("localhost:") && !host?.includes("localhost")) {
      url.host = host as string;
      url.port = "";
    }
    ...
    if (callbackUrl) {
      const callbackNextPath = callbackUrl.searchParams.get("next");
      nextPath = callbackNextPath ? callbackNextPath : callbackUrl.pathname;
    }
